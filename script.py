...
def reset_for_new_innings():
    ...
    st.session_state.current_bowler = st.selectbox("Opening Bowler", 
                                                 [p for p in player_names if p not in [st.session_state.current_batter, st.session_state.runner]], 
                                                 key="bowler_select_2")
...
def show_match_summary():
    ...
    # Result
    st.success(f"**Result**: {determine_winner()} won by "
              f"{abs(st.session_state.first_innings_score - st.session_state.score)} runs")
    ...
    st.session_state.mom = st.selectbox("Man of the Match", set(players))
...
def player_stats():
    ...
    selected_player = st.selectbox("Select player:", df["name"].values)
    if selected_player:
        ...
        with col2:
            ...
            if player.bowler_balls > 0:
                st.metric("Bowling Average", round(player.bowler_runs/player.wickets, 2) if player.wickets > 0 else "-")
        ...
        batting_fig = px.bar(
            x=["Runs", "Fours", "Sixes"],
            y=[int(player.runs), int(player.fours), int(player.sixes)],
        ...
        bowling_fig = px.bar(
            x=["Wickets", "Runs Conceded"],
            y=[int(player.wickets), int(player.bowler_runs)],
            labels={'x': 'Stat', 'y': 'Count'}
        )
...
def match_setup():
    ...
    with col1:
        st.session_state.current_batter = st.selectbox("Striker", player_names, key="striker_select")
    with col2:
        st.session_state.runner = st.selectbox("Non-Striker", 
                                             [p for p in player_names if p != st.session_state.current_batter], 
                                             key="runner_select")
...
def show_scoring():
    ...
    with col2:
        st.subheader(f"Target: {st.session_state.target if st.session_state.innings == 2 else '-'}")
    with col3:
